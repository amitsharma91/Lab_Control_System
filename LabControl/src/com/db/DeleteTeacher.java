...
public class DeleteTeacher extends JFrame implements ActionListener{
	...
	public DeleteTeacher() {
		...
		table.setModel(new DefaultTableModel(
				records,
			new String[] {
				"Teacher ID", "Teacher Name", "Username"
			}
		));
		table.enable(false);
...
//			System.out.println("Rows : "+rows_no);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (selectedOption == JOptionPane.YES_OPTION) {
			...
			try{
				int res = DBManager.getConnection().createStatement().executeUpdate("DELETE FROM teacher WHERE tid = "+id); 
				if(res>0){
					JOptionPane.showMessageDialog(null, "Faulty Deleted Sucessfully..");
					records = getData();		
					table.setModel(new DefaultTableModel(
							records,
						new String[] {
