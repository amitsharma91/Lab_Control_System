...
public class TeacherMainConsole extends JFrame {
	...
	public TeacherMainConsole(String username) {
		...
		setResizable(false);
		setIconImage(new ImageIcon(TeacherMainConsole.class.getResource("/img/lab.png")).getImage());
		setVisible(true);
							...
							public void run() {
								while (true) {
									...
									try {
										...
										System.out.println("Some Problem in getting USB-Detection-Info....\n" + e);
										break;
										/*try {
											Thread.sleep(3000);
											System.exit(0);
		...
		SwingWorker<String, String> workerApplication = new SwingWorker<String, String>() {
			...
			protected String doInBackground() throws Exception {
				...
				try {
					serverSocApp = new ServerSocket(9511);// server Socket
					System.out.println("Waiting for Client...");
				} catch (IOException e) {
					e.printStackTrace();
				}
							...
							public void run() {
								while (true) {
									...
									try {
										...
										System.out.println(incommingMsg);
										if (incommingMsg.contains("app")) {
											System.out.println("Client Msg>>> " + incommingMsg);
											new NotificationPopUpForApplications(incommingMsg);
										}
									} catch (Exception e) {
										System.out.println("Some problem geeting App-Detection-Info....\n" + e);
										break;
										/*try {
											Thread.sleep(3000);
											System.exit(0);
